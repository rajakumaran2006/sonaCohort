...
function PeerSubjectDetailsContent() {
   ...
   const getCompletionStatus = (scheduledClass: ScheduledClassWithDetails) => {
      if (scheduledClass.completion_status === 'completed' ||
         ...
         return { status: 'completed', color: 'bg-emerald-600 text-white border-emerald-100', label: 'Completed' }
      } else if (scheduledClass.completion_status === 'pending' ||
         scheduledClass.attendance_completed || scheduledClass.topics_completed) {
         return { status: 'pending', color: 'bg-amber-600 text-white border-amber-100', label: 'Pending' }
      } else {
         return { status: 'not_started', color: 'bg-gray-600 text-white border-gray-100', label: 'Not Started' }
      }
   }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                           ...
                           <Card className="p-6 rounded-[2rem] border-none shadow-sm bg-white relative overflow-hidden group hover:shadow-md transition-all duration-300">
                              <div className="relative z-10">
                                 ...
                                 <h3 className="text-3xl font-black text-gray-900 tracking-tight mb-2">{totalClasses}</h3>
                           ...
                           <Card className="p-6 rounded-[2rem] border-none shadow-sm bg-white relative overflow-hidden group hover:shadow-md transition-all duration-300">
                              <div className="relative z-10">
                                 ...
                                 <h3 className="text-3xl font-black text-gray-900 tracking-tight mb-2">{completedClasses}</h3>
                           ...
                           <Card className="p-6 rounded-[2rem] border-none shadow-sm bg-white relative overflow-hidden group hover:shadow-md transition-all duration-300">
                              <div className="relative z-10">
                                 ...
                                 <h3 className="text-3xl font-black text-gray-900 tracking-tight mb-2">{pendingClasses}</h3>
                        ...
                        {fullReport && (
                           <Card className="rounded-[2rem] shadow-sm border-none bg-white p-7 overflow-hidden">
                              <div className="flex flex-row items-center justify-between mb-8 border-b border-gray-50 pb-4">
                                 ...
                                 <div className="text-xs font-bold text-gray-500 uppercase tracking-wider flex items-center gap-4">
                                    <div>Subject: <span className="text-gray-900">{fullReport.subject_name}</span></div>
                                             ...
                                             {fullReport.columns.slice(-5).map(col => {
                                                ...
                                                return (
                                                   <div key={col.id} className="flex flex-col items-center gap-1 w-full">
                                                      <div
                                                         className={`w-full h-1.5 rounded-full ${status === 'present' ? 'bg-emerald-500' :
                                                            status === 'absent' ? 'bg-red-500' :
                                                               status === 'on_duty' ? 'bg-blue-500' :
                                                                  status === 'upcoming' ? 'bg-gray-400' : 'bg-gray-200'
                                                            }`}
                                                      />
                                             ...
                                             <td className="py-4 pr-4 align-top text-right">
                                                {isCompleted ? (
                                                   <button
                                                      ...
                                                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 rounded-lg text-[10px] font-black text-black uppercase tracking-wider hover:bg-blue-50 hover:border-blue-100 hover:text-blue-700 transition-all shadow-sm"
                                                   >
                                                      View
                                                   </button>
                                                ) : (
               ...
               <div className="relative mx-auto w-full max-w-4xl shadow-2xl rounded-[2rem] bg-white border border-gray-100 overflow-hidden">
                  ...
                  <div className="p-8 space-y-8">
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                           <p className="text-[10px] font-bold text-black uppercase tracking-wider mb-1">Present Students</p>
                           <div className="flex items-baseline gap-2">
                              <span className="text-2xl font-black text-black">{selectedClass.present_count}</span>
                              <span className="text-xs font-bold text-black uppercase">Students</span>
                           </div>
                        </div>
                        <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                           <p className="text-[10px] font-bold text-black uppercase tracking-wider mb-1">Absent Students</p>
                           <div className="flex items-baseline gap-2">
                              <span className="text-2xl font-black text-black">{selectedClass.absent_count}</span>
                              <span className="text-xs font-bold text-black uppercase">Students</span>
                           </div>
                        </div>
                     </div>
