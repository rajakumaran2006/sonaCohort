...
const PixelBlast = ({
  ...
  const threeRef = useRef<{
    ...
    touch?: {
      update: () => void;
      radiusScale: number;
      addTouch: (p: { x: number; y: number }) => void;
    };
    liquidEffect?: Effect & { uniforms: Map<string, THREE.IUniform> };
  } | null>(null);
  const prevConfigRef = useRef<Record<string, boolean | number | string> | null>(null);
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    speedRef.current = speed;
    const needsReinitKeys = ['antialias', 'liquid', 'noiseAmount'];
    const cfg: Record<string, boolean | number | string> = { antialias, liquid, noiseAmount };
    ...
    if (mustReinit) {
      ...
      const mapToPixels = (e: PointerEvent | React.PointerEvent) => {
      ...
      const onPointerDown = (e: PointerEvent | React.PointerEvent) => {
        ...
        if (composer) {
          ...
          composer.passes.forEach((p) => {
            if (p instanceof EffectPass) {
              const effs = (p as unknown as Record<string, unknown>).effects as Effect[] | undefined;
              if (effs)
                effs.forEach((eff) => {
