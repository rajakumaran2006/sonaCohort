...
export default function ClassesImportExport({ 
  ...
  const handleExportData = async () => {
    try {
      ...
      const exportData = [
        ...
        ['3. Start/End Time: Optional, use HH:MM format (24h)'],
        ['4. Link: Optional, meeting link'],
        ['5. Topics: Optional description of class topics'],
        ['6. Action: UPDATE (modify existing) or CREATE (new subject)'],
        ['7. UPDATE: Add new schedule to existing subject'],
        ['8. CREATE: Create new subject and schedule it'],
        [],
        
        // Data headers
        // Data headers
        ['Subject Name', 'Scheduled Date (YYYY-MM-DD)', 'Start Time (HH:MM)', 'End Time (HH:MM)', 'Link (Optional)', 'Topics (Optional)', 'Action (UPDATE/CREATE)'],
        []
      ]
      ...
      exportData.push(['Mathematics', tomorrow.toISOString().split('T')[0], '10:00', '11:00', 'https://meet.google.com/abc', 'Algebra and Calculus', 'CREATE'])
      exportData.push(['Physics', dayAfter.toISOString().split('T')[0], '14:00', '15:30', '', 'Mechanics and Thermodynamics', 'CREATE'])
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        if (row && row.length >= 7 && row[0] === 'Subject Name' && row[1] === 'Scheduled Date (YYYY-MM-DD)' && row[6] === 'Action (UPDATE/CREATE)') {
          dataStartIndex = i + 1
          break
        }
      }
  ...
  const validateImportData = async (rows: string[][]): Promise<ImportPreview> => {
    ...
    for (const row of rows) {
      if (row.length < 4) continue // Allow fewer than 7 if intermediate optional cols are missing? No, safer to check essential checks.
      
      // Ensure we have enough columns or pad it?
      // With header: Subject, Date, Start, End, Link, Topics, Action
      // Indices: 0, 1, 2, 3, 4, 5, 6
      const subjectName = row[0]
      const scheduledDate = row[1]
