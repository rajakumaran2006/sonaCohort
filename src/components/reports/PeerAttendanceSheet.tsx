                ...
                <div key={student.student_id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                  <div className="flex justify-between items-start mb-4 border-b border-gray-100 pb-3">
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                      student.stats.percentage >= 75 ? 'bg-green-100 text-green-700' : 
                      student.stats.percentage >= 60 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
                    }`}>
                  ...
                  <div className="flex items-center gap-4 mb-4 text-xs">
                    <div className="bg-green-50 px-3 py-2 rounded-lg flex-1 text-center">
                      <span className="block text-gray-500 font-bold uppercase text-[10px] mb-0.5">Present</span>
                      <span className="text-lg font-bold text-green-700">{student.stats.present}</span>
                    </div>
                    <div className="bg-gray-50 px-3 py-2 rounded-lg flex-1 text-center">
                      <span className="block text-gray-500 font-bold uppercase text-[10px] mb-0.5">Total</span>
                      <span className="text-lg font-bold text-gray-900">{subject.columns.length}</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {subject.columns.map(col => {
                        ...
                        return (
                          <div key={col.id} className={`flex flex-col items-center p-2 rounded-lg border ${
                            status === 'present' ? 'bg-green-50 border-green-100' :
                            status === 'absent' ? 'bg-red-50 border-red-100' :
                            status === 'on_duty' ? 'bg-purple-50 border-purple-100' : 
                            'bg-gray-50 border-gray-100'
                          }`}>
                            <span className="text-[10px] text-gray-500 font-medium mb-1">{formattedDate}</span>
                            <span className={`text-xs font-bold ${
                              status === 'present' ? 'text-green-700' :
                              status === 'absent' ? 'text-red-700' :
                              status === 'on_duty' ? 'text-purple-700' : 
                              'text-gray-400'
                            }`}>
                              {status === 'present' ? 'P' : status === 'absent' ? 'A' : status === 'on_duty' ? 'OD' : '-'}
                            </span>
